...
require 'zigbee/zdo/match_descriptor_response'

describe Zigbee::ZDO::MatchDescriptorResponse do

  it "decodes with endpoint list" do
    bytes = [ 0x00, 0x11, 0x22, 0x02, 0x55, 0x66 ]
    z = Zigbee::ZDO::MatchDescriptorResponse.decode(bytes)
  ...
  it "throws if an odd number of bytes remain" do
    ...
    expect {
      Zigbee::ZDO::MatchDescriptorResponse.decode(values)
    }.to raise_error(ArgumentError)
  end

  it "encodes with endpoints" do
    z = Zigbee::ZDO::MatchDescriptorResponse.new(0x00, 0x1122, [ 0x55, 0x66 ])
    expect(z.encode).to eq([ 0x00, 0x22, 0x11, 0x02, 0x55, 0x66 ])
  end

  it "encodes without endpoints" do
    z = Zigbee::ZDO::MatchDescriptorResponse.new(0x00, 0x1122, [])
